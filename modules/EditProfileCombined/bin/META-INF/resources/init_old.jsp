...
.formGroupmb0.showIcon,
.formGroupmb0.firstFormGroup:not(.active):not(.showIcon) > a,
            	   ...
            	   complete:function()
                    ...
                    {			
                    	...
                    	if(value!=null && value!="undefined" && value!=undefined){
                    		...
                    		$(".view-discipline").text(data.discipline1);
                    		...
                    		$(".view-location").text(data.region1);
                    		...
                    		$(".editInterest").on("click", function(){ debugger;
                    			...
                    			if(data.collaborationType === "Global") {
                    				...
                    				$('.selectableDR.selectableDREdit').hide();
                    			}
                    			if(data.region1 === "All") {
                    				$("[id^='<portlet:namespace/>locationEdit']").parents('.selectableDR').removeClass("active");
                    			}
                    			...
                    			$("#projectRange2").ionRangeSlider({ to: data.endMonth});
                    			$("#rangerDatepicker2").val(data.endYear);
                    			console.log("data.endMonth2", data.endMonth);
                    			console.log("data.endYear2", data.endYear);
...
function saveProfile(buttonRequest){
	...
	var monthRange = $(".js-irs-2 span.irs-single").text();
	$("#<portlet:namespace/>endMonthRangeAdd").val(monthRange);
...
function resetAddInterest() {
	...
	$("#add-interest-modal .selectableDR.collaborationShow, #add-interest-modal #courseDevelopmentPopup").hide();
	// $(".js-range-slider").data("ionRangeSlider").destory();
	$("#rangerDatepicker3").val(nd);
...
function updateAreaofInterest(){
	...
	$("#<portlet:namespace/>endMonthRangeEdit").val(monthRangeEdit);
...
$(document).ready(function(){
	...
	$("[id^='<portlet:namespace/>discipline'], [id^='<portlet:namespace/>location']").on("change", function(){
		$(this).parents('.selectableDR').removeClass("active");
		$(this).parents('.selectableDR').next().addClass("active");
	});
	$("#<portlet:namespace/>location1, #<portlet:namespace/>locationEdit1").on("change", function(){ debugger;
	...
	$("[id^='<portlet:namespace/>collaborationType']").on("change", function(){ debugger;
		...
		if(selectedCollaborationType === "Global") {
			$('.selectableDR.collaborationShow, .selectableDR.selectableDREdit').show();
		} else {
			$('.selectableDR.collaborationShow, .selectableDR.selectableDREdit').hide();
		}
	});
