...
public class PersonalInformationPortlet extends MVCPortlet {
	    ...
	    if (i > -1) {
	       ...
	       for (String param : params) {
			   ...
			   try {
				   ...
				   ps.setAttribute("LIFERAY_SHARED_USER_TOKEN", userToken, PortletSession.APPLICATION_SCOPE);
	...
	public void getProfileInfo(ResourceRequest resourceRequest, ResourceResponse resourceResponse){
		...
		try{
			...
			if(interestId!=0){
				...
				dynamicQuery.add(RestrictionsFactoryUtil.and(RestrictionsFactoryUtil.eq("inviteTo",user.getUserId()),RestrictionsFactoryUtil.eq("projectId", interestId)));
		    ...
		    if (i > -1) {
		       ...
		       for (String param : params) {
		           ...
		           System.out.println("userToken==="+resourceResponse.getNamespace());
		           if(userToken.equalsIgnoreCase((resourceResponse.getNamespace().substring(1, resourceResponse.getNamespace().length()-1)))){
	...
	public void getViewMoreDataInvite(ResourceRequest resourceRequest, ResourceResponse resourceResponse) {
		...
		User user = null;
		String prefixValue = "";
		String profileStatus = "",deliveryMethod="";
		try{
			out = resourceResponse.getWriter();
			jsonObject = JSONFactoryUtil.createJSONObject();
	...
	public void sendInvite(ResourceRequest resourceRequest, ResourceResponse resourceResponse){
		...
		try{
			...
			if(inviteTo!=0){
				...
				DynamicQuery dynamicQuery = DynamicQueryFactoryUtil.forClass(projectInviteTracking.class, PortalClassLoaderUtil.getClassLoader());
				dynamicQuery.add(RestrictionsFactoryUtil.and(RestrictionsFactoryUtil.eq("inviteTo",selectedProfileMatching),RestrictionsFactoryUtil.eq("projectId", interestId)));
				List<projectInviteTracking> projectInviteTrackingList = projectInviteTrackingLocalServiceUtil.dynamicQuery(dynamicQuery);
				if(projectInviteTrackingList.size()>0){
					 isAlreadySent = true;
				}
